...
with registry.cursor() as cr:
    ...
    for emp in employees:
        if not emp.user_id:
            ...
            user_map[emp.name] = user
            print(f"LINKED USER FOR {emp.name}: {user.login}")
        else:
            user_map[emp.name] = emp.user_id
    ...
    for task_id, names in assignments:
        ...
        if task.exists():
            ...
            print(f"SUCCESS! ASSIGNED ASSIGNEES TO TASK {task.id} ({task.name}): {names}")

    cr.commit()
    print("ALL 6 TASKS UPDATED WITH REAL USER ASSIGNEES!")
